...
getRecieverDetails(){
  ...
  db.collection('requestedItem').where('requestId','==',this.state.requestId).get()
  .then(snapshot=>{
    snapshot.forEach(doc => {
...
updateBookStatus=()=>{
  db.collection('allTranstions').add({
...
addNotification=()=>{
  ...
  var msg=this.state.userId+" Has shown intresed in Donating Book"
      ...
      <View style={styles.container}>
        <View style={{flex:0.1}}>
          <Header
            ...
            centerComponent={{ text:"Buy Items", style: { color: '#90A5A9', fontSize:20,fontWeight:"bold", } }}
            ...
            ?(
              <TouchableOpacity
                  ...
                  onPress={()=>{
                    ...
                    this.props.navigation.navigate('myTranstionsScreen')
                  }}>
