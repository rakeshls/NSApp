...
export default class Sell extends Component {
  ...
  addItem = (itemName, description,itemPrice) => {
  ...
  sendNotification=()=>{
    ...
    .then((snapshot)=>{
      snapshot.forEach((doc)=>{
        ...
        db.collection('allNotifications').where('exchageId','==',this.state.requestId).get()
        .then((snapshot)=>{
          snapshot.forEach((doc) => {
  ...
  render() {
    return (
      ...
      <View style = {{ flex: 1}}>
        ...
        <MyHeader navigation={this.props.navigation} title = "Sell items"/>
        ...
        <View style={{ flex: 1 }}>
          ...
          <TextInput
            ...
            <TouchableOpacity
              style={styles.button}
              onPress={() => {
                this.addItem(this.state.itemName, this.state.description, this.state.itemPrice);
              }}
            >
              <Text> Add Item </Text>
            </TouchableOpacity>
